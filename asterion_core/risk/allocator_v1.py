    ...
    *,
    ...
    if not market_station_lookup:
        market_station_lookup = build_market_station_map(
            con,
            market_ids=[str(item.market_id) for item in decisions if item.market_id],
        )
    ...
    exposure_basis = _load_current_exposure_basis(
        ...
        reference_price_by_market={str(item.market_id): item.reference_price for item in decisions},
    ...
    for decision in decisions:
        ...
        if str(decision.side).lower() == "buy":
            requested_buy_notional_total += requested_notional
        ...
        if policy is None:
            ...
            allocation_decisions.append(allocation_decision)
            continue

        policy_id = str(policy["policy_id"])
        policy_version = str(policy["policy_version"])
        min_recommended_size = max(Decimal("0"), _decimal(policy["min_recommended_size"]))
        rounding_increment = max(Decimal("0.00000001"), _decimal(policy["size_rounding_increment"]))
        per_ticket_buy_cap = max(Decimal("0"), _decimal(policy["max_buy_notional_per_ticket"]))
        remaining_run_budget = policy_remaining_budget.get(policy_id, Decimal("0"))
        ...
        limit_checks = _evaluate_position_limits(
            decision=decision,
            allocation_run_id=allocation_run_id,
            wallet_id=wallet_id,
        ...
        allocation_decision = AllocationDecision(
            ...
            budget_impact={
                ...
                "remaining_run_budget": float(policy_remaining_budget.get(policy_id, Decimal("0")))
                if str(decision.side).lower() == "buy"
                else None,
    ...
    strategy_ids = {item.strategy_id for item in decisions}
