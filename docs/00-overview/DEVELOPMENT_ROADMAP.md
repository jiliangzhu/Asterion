...
- `P2` 已关闭
- `P3` 可开工
- 当前开发入口是 `paper execution / readiness-driven operator loop`
- `P1` 的 canonical closeout 文档见：
  - [P1_Closeout_Checklist.md](../10-implementation/checklists/P1_Closeout_Checklist.md)
  - [P1_Watch_Only_Replay_Cold_Path_Runbook.md](../10-implementation/runbooks/P1_Watch_Only_Replay_Cold_Path_Runbook.md)
- `P2` 的 canonical 开工文档见：
  - [P2_Implementation_Plan.md](../10-implementation/phase-plans/P2_Implementation_Plan.md)
- `P2` 的 canonical closeout 文档见：
  - [P2_Closeout_Checklist.md](../10-implementation/checklists/P2_Closeout_Checklist.md)
...
- [P1_P2_AlphaDesk_Remaining_Migration_Checklist.md](../10-implementation/checklists/P1_P2_AlphaDesk_Remaining_Migration_Checklist.md) 中 `health_monitor_v1` / `ui_db_replica` 已迁入，或已完成 Asterion 原生替代
...
1. 在 deterministic Rule2Spec parser 之上实现 `Rule2Spec Agent`
2. 在 replay / provenance / fallback 基础上实现 `Data QA Agent`
3. 在 watcher replay / settlement verification / evidence linkage 基础上实现 `Resolution Agent`
4. 所有 agent 输出都保持 human-in-the-loop，不进入执行路径
...
- agent 失败不会阻塞 replay / pricing / watcher 主链路
...
## 8. P4: Execution Foundation（不下真实单）

**目标**: 把 execution path 的 contract 和只读能力接上，为 paper execution 铺路。

**预计时间**: 2 周

### 7.1 工作包 A: Capability Registry

前置依赖：

- [P1_P2_AlphaDesk_Remaining_Migration_Checklist.md](../10-implementation/checklists/P1_P2_AlphaDesk_Remaining_Migration_Checklist.md) 中 Wave B runtime skeleton 已迁入，或 Asterion 已完成对应原生实现
- `execution_gate / journal / portfolio` 不再需要通过阅读 AlphaDesk 源码来补实现

1. 实现 `MarketCapabilityProvider`
2. 实现 `AccountCapabilityProvider`
3. 实现 `ExecutionContextBuilder`
4. token 级读取 `fee_rate_bps / tick_size / neg_risk`

### 7.2 工作包 B: CLOB read-only path

1. 实现 orderbook snapshots
2. 实现 liquidity estimator
3. 实现 slippage model
4. 实现 route decision logging

### 7.3 工作包 C: OMS skeleton

1. 实现 orders / fills / reservations / inventory positions 的持久化壳
2. 实现 reservation 计算逻辑
3. 实现 exposure snapshots
4. 实现 reconciliation skeleton
5. 适配 AlphaDesk 的 journal / queue / write path 到 Asterion OMS 事件

### 7.4 交付物

- 可读 market/account capability
- 可生成 `ExecutionContext`
- 可记录 route decisions
- 可落库 inventory skeleton

### 7.5 验收条件

- Router 不再绕过 capability registry
- reservation 逻辑能按 `price * size` 运行
- inventory key 不依赖 `YES/NO`

---

## 9. P5: Paper Execution

**目标**: 在不动真资金的前提下，把 execution path 完整走通。

**预计时间**: 3-4 周

### 8.1 工作包 A: Signer and account plumbing

1. 实现 signer service RPC skeleton
2. 实现 `derive_api_credentials(wallet_ref)`
3. 接入官方 py-clob-client order signing
4. 接入 account capability checks

### 8.2 工作包 B: Router -> OMS -> Adapter

前置依赖：

- Wave B runtime / execution skeleton 已完成 Asterion 适配

1. 实现 `RouteAction` 到 adapter mapping
2. 实现 order creation / reservation / post / fill / cancel / expire 状态机
3. 接入 paper exchange simulator 或 mock fills
4. 记录 execution journal

### 8.3 工作包 C: Inventory / reconciliation

1. BUY reservation 到 fill conversion
2. SELL token reservation 到 proceeds conversion
3. open orders / fills / internal state reconciliation
4. exposure snapshots 与 PnL 辅助视图

### 8.4 交付物

- 端到端 paper execution
- 完整 OMS 状态流
- inventory / reservation / reconciliation 运行

### 8.5 验收条件

- Paper order 能完整经历 created -> reserved -> posted -> partial/full -> released/converted
- paper fills 能稳定更新 inventory
- reconciliation 不出现系统性漂移

---

## 10. P6: Live Prerequisites
...
1. 完成 signer 隔离部署
2. 完成 multi-RPC 生产配置
3. 完成 CLOB / chain / inventory 监控告警
4. 完成 circuit breaker 与 risk gate 接入 execution
5. 完成 operator runbook
6. 完成 small-capital dry run checklist

### 9.2 交付物

- live readiness checklist
- operator incident runbook
- signer deployment plan
- kill-switch / review gates

### 9.3 进入 live 前的强制门槛

- 连续至少 2 周 watch-only 稳定运行
- replay 与 backfill 稳定
- paper execution 连续通过验收
- watcher / verifier / redeem review 流程稳定
- human approval 流程可用
