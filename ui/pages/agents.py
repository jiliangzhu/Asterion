...
def show() -> None:
    ...
    st.caption("这里优先展示每个 agent 做了哪些工作、哪些对象需要人工复核，配置只作为辅助信息。")

    top1, top2, top3 = st.columns(3)
    with top1:
        st.metric("Provider", status["provider"], delta="effective runtime")
    with top2:
    ...
    if review_frame.empty:
        ...
        st.dataframe(review_frame[columns].head(20), width="stretch", hide_index=True)
